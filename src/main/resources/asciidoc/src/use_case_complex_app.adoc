...
* The VersionSet is mapped to application (microservice) and scope (stack, region, and environment). For each application, a mapping is created for each relevant environment (such as *dev* and *prod*), region (such as *us-west-2*), and stack (such as *supersite* and *productpage*).
