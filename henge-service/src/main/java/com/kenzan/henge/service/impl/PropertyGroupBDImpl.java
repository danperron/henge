...
import com.kenzan.henge.domain.legacy.converter.Converter;
import com.kenzan.henge.domain.legacy.converter.PropertyGroupConverter;
...
public class PropertyGroupBDImpl implements PropertyGroupBD {
	...
	@Override
	public PropertyGroup importLegacyPropertyGroup(LegacyPropertyGroup entity) {
		Converter<LegacyPropertyGroup, PropertyGroup> converter = new PropertyGroupConverter();
		
		PropertyGroup propertyGroup = converter.convert(entity);
		Optional<PropertyGroup> persistedEntity = update(propertyGroup.getName(), propertyGroup); 
		if (!persistedEntity.isPresent()) {
		    persistedEntity = Optional.of(create(propertyGroup));
		}
		
		return persistedEntity.get();
	}
